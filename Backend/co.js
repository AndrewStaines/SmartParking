...
app.post('/run-python', (req, res) => {
  const pythonProcess = spawn('python', ["C:/Users/andre/Downloads/smart-parking-2-main/main/src/assets/sp final.py"]);
  let output = '';  // Initialize a variable to store the output
  ...
  pythonProcess.on('close', (code) => {
    if (code === 0) {
      // Process the output once the Python script has finished running
      output = output.trim();  // Remove leading/trailing whitespace
      const parsedOutput = parseInt(output, 10);  // Parse as an integer
      if (!isNaN(parsedOutput)) {
        res.send(parsedOutput.toString());  // Send the parsed output
      } else {
        res.status(500).send('Invalid output');
      }
    } else {
